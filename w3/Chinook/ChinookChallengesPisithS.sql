...
-- The DISTINCT keyword is a way to retrieve unique value of whatever information we are asking for

SELECT DISTINCT BillingCountry FROM Invoice;

-- Retrieve how many invoices there were in 2009, and what was the sales total for that year?

SELECT COUNT(*) AS 'Total Count from 2009', SUM(Total) as 'Total 2009' FROM Invoice
...
SELECT COUNT(*) AS 'Line Item' FROM InvoiceLine
WHERE InvoiceId = 37;

-- how many invoices per country?

SELECT BillingCountry, COUNT(InvoiceId) AS 'Total Invoice' FROM Invoice
GROUP BY BillingCountry;

-- Retrieve the total sales per country, ordered by the highest total sales first.

SELECT BillingCountry, SUM(Invoice) AS Total FROM Invoice
GROUP BY BillingCountry
ORDER BY Total DESC;
